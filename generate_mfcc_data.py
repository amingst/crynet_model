...
def generate_mfcc_data(data_path):
    ...
    for row in audio_df.iterrows():
        ...
        mfcc_data.append([mfccs, scaled_mfccs, current_row.class_label])
    
    df = pd.DataFrame(mfcc_data, columns=["mfccs", "scaled_mfccs", "class_label"])
    df.to_pickle('mfcc_data.pkl')

    print("Saved to pickle file in root directory")
    return None

generate_mfcc_data('audio_data.pkl')
